<template>
  <VCard class="ma-5" elevation="12">
    <VRow class="ma-1 mb-0">
      <h2 class="v-col-12 text-center formTitle">{{ app[getcurrentLanguge()].property.typeProduct.title }}</h2>
        ...
        <v-table fixed-header hover density="comfortable" class="custom-table" loading
          ...
          <thead>
            <tr>
              ...
              <th class="text-center">
                {{ app[getcurrentLanguge()].property.typeProduct.attribute.name }}
              </th>
              <th class="text-center">
                {{ app[getcurrentLanguge()].property.typeProduct.attribute.action }}
              </th>
            </tr>
          </thead>
          <tbody>
...
import { getAll, save, update } from '@/services/typeService'
  ...
  if (currentItem.value.id) {
    await update(currentItem.value).then(resp => {
      if (resp.status >= 200 && resp.status < 300) {
        ...
        toast.info(app[getcurrentLanguge()].property.typeProduct.action.updateSuccess);
        close();
      } else {
        toast.error(app[getcurrentLanguge()].property.typeProduct.action.updatefaild);
      }
    }).catch((err) => {
      toast.error(app[getcurrentLanguge()].networkFaild);
    })
    ...
    await save(currentItem.value).then(resp => {
      if (resp.status >= 200 && resp.status < 300) {
        getData(page.value - 1, pageSize.value);
        toast.info(app[getcurrentLanguge()].property.typeProduct.action.newSuccess);
        close();
      } else {
        toast.error(app[getcurrentLanguge()].property.typeProduct.action.newFaild);
      }
    }).catch((err) => {
      toast.error(app[getcurrentLanguge()].networkFaild);
    })
