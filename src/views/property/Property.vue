<template>
  <VCard class="ma-5" elevation="12">
    <VRow class="ma-1 mb-0">
      <h2 class="v-col-12 text-center formTitle">{{ app[getcurrentLanguge()].property.property.title }}</h2>
        ...
        <v-table fixed-header hover density="comfortable" class="custom-table" loading
          ...
          <thead>
            <tr>
              ...
              <th class="text-center">
                {{ app[getcurrentLanguge()].property.property.attribute.name }}
              </th>
              <th class="text-center">
                {{ app[getcurrentLanguge()].property.property.attribute.action }}
              </th>
            </tr>
          </thead>
          <tbody>
          ...
          <v-row>
            <v-col cols="12">
              <v-text-field v-model="currentItem.name"
                :label="app[getcurrentLanguge()].property.property.attribute.name"></v-text-field>
            </v-col>
          </v-row>
        </v-container>
      </v-card-text>
...
import { getAll, save, update } from '@/services/propertyService'
...
const editItem = (id) => {
  ...
  formTitle.value = app[getcurrentLanguge()].property.property.action.update;
  dialog.value = true;
}
const infor = (id) => {
  ...
  if (currentItem.value.id) {
    await update(currentItem.value).then(resp => {
      if (resp.status >= 200 && resp.status < 300) {
        ...
        toast.error(app[getcurrentLanguge()].property.property.action.updateSuccess);
        close();
      } else {
        toast.error(app[getcurrentLanguge()].property.property.action.updatefaild);
      }
    }).catch((err) => {
      toast.error(app[getcurrentLanguge()].networkFaild);
    })
    ...
    await save(currentItem.value).then(resp => {
      ...
      if (resp.status >= 200 && resp.status < 300) {
        getData(page.value - 1, pageSize.value);
        toast.info(app[getcurrentLanguge()].property.property.action.newSuccess);
        close();
      } else {
        toast.error(app[getcurrentLanguge()].property.property.action.newfaild);
      }
    }).catch((err) => {
      toast.error(app[getcurrentLanguge()].networkFaild);
    })
