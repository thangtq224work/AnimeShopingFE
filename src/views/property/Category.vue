<template>
    <VCard class="ma-5" elevation="12">
        <VRow class="ma-1 mb-0">
            <h2 class="v-col-12 text-center formTitle">{{ app[getcurrentLanguge()].property.category.title }}</h2>
                ...
                <v-table fixed-header hover density="comfortable" class="custom-table" loading
                    ...
                    <thead>
                        <tr>
                            ...
                            <th class="text-center">
                                {{ app[getcurrentLanguge()].property.category.attribute.name }}
                            </th>
                            <th class="text-center">
                                {{ app[getcurrentLanguge()].property.category.attribute.action }}
                            </th>
                        </tr>
                    </thead>
                    <tbody>
...
import { getAll, save, update } from '@/services/categoryService'
...
const editItem = (id) => {
    ...
    formTitle.value = app[getcurrentLanguge()].property.category.action.update;
    dialog.value = true;
}
const infor = (id) => {
    ...
    if (currentItem.value.id) {
        await update(currentItem.value).then(resp => {
            if (resp.status >= 200 && resp.status < 300) {
                ...
                toast.info(app[getcurrentLanguge()].property.category.action.updateSuccess);
                close();
            }
            else {
                toast.error(app[getcurrentLanguge()].property.category.action.updatefaild);
            }
        }).catch((err) => {
            toast.error(app[getcurrentLanguge()].networkFaild);
        })
        ...
        await save(currentItem.value).then(resp => {
            if (resp.status >= 200 && resp.status < 300) {
                ...
                toast.error(app[getcurrentLanguge()].property.category.action.newfaild);
            }
        }).catch((err) => {
            toast.error(app[getcurrentLanguge()].networkFaild);
        })
