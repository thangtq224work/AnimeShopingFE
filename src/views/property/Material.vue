<template>
  <VCard class="ma-5" elevation="12">
    <VRow class="ma-1 mb-0">
      <h2 class="v-col-12 text-center formTitle">{{ app[getcurrentLanguge()].property.material.title }}</h2>
        ...
        <v-table fixed-header hover density="comfortable" class="custom-table" loading
          ...
          <thead>
            <tr>
              ...
              <th class="text-center">
                {{ app[getcurrentLanguge()].property.material.attribute.name }}
              </th>
              <th class="text-center">
                {{ app[getcurrentLanguge()].property.material.attribute.action }}
              </th>
            </tr>
          </thead>
          <tbody>
          ...
          <v-row>
            <v-col cols="12">
              <v-text-field v-model="currentItem.name" :rules="nameValidate"
                :label="app[getcurrentLanguge()].property.material.attribute.name"></v-text-field>
            </v-col>
            <v-col cols="12">
...
import { getAll, save, update } from '@/services/materialSerivice'
...
const statusValidate = [
  (value) => {
    ...
    return app[getcurrentLanguge()].validate.material.statusNotNull;
  }
];
...
const editItem = (item) => {
  ...
  formTitle.value = app[getcurrentLanguge()].property.material.action.update;
  dialog.value = true;
}
const infor = (id) => {
  ...
  if (currentItem.value.id) {
    await update(currentItem.value).then(resp => {
      if (resp.status >= 200 && resp.status < 300) {
        ...
        toast.info(app[getcurrentLanguge()].property.material.action.updateSuccess);
        close();
      } else {
        toast.error(app[getcurrentLanguge()].property.material.action.updatefaild);
      }
    }).catch((err) => {
      toast.error(app[getcurrentLanguge()].networkFaild);
    })
    ...
    await save(currentItem.value).then(resp => {
      if (resp.status >= 200 && resp.status < 300) {
        getData(page.value - 1, pageSize.value);
        toast.info(app[getcurrentLanguge()].property.material.action.newSuccess);
        close();
      } else {
        toast.error(app[getcurrentLanguge()].property.material.action.newfaild);
      }
    }).catch((err) => {
      toast.error(app[getcurrentLanguge()].networkFaild);
    })
